...
Status: Rejected for sustained 1,000 RPS; retained as measured experimental evidence.
...
The 15-minute recovery-SLO diagnostic at revision `0a45cb9` did not sustain 1,000 RPS and blocks a 30-minute promotion. Of 900,000 scheduled requests, 14,939 were generator drops and 887,941 physical attempts were made. First attempts saw 2,862 hold admission failures, one hold timeout and 17 read timeouts; retry recovered 698 holds and 15 reads, while 2,165 holds and two reads were exhausted. Admission attempts were rejected 5,021 times. Worst-worker read and hold p95 rose to 501.744 ms and 338.834 ms.

The audit found 42,100 acknowledged holds and zero overlapping intervals or broken links. Two worker runs each contained one more durable idempotency record, hold and order than acknowledged 201 responses, consistent with a commit succeeding before the client timeout; therefore exact acknowledged-response accounting failed even though no committed record was missing. Refresh generation and completion both reached 85,514, pending refresh ended at zero, Kafka maximum total lag was 70 and ended at zero, and all queues drained. The RDS observer saw up to 17 simultaneous interesting waiters dominated by `WALWrite` and `WalSync`, with 963.3 MB of WAL generated. Rollback restored admission four, one maintenance worker and one consumer.

This evidence rejects three maintenance workers as a sustained 1,000 RPS solution on the measured database budget. Further load is stopped. The next change must reduce or isolate WAL commit concurrency, such as separating refresh from expiry scheduling or batching refresh persistence, and requires a new ADR before implementation.
