...
A direct 1,000 RPS diagnostic on the same Huawei topology used the same retry settings and a 95% read / 5% hold mix. The five-minute stage passed all gates: 300,000 logical and physical requests, no drops or retry attempts, read/hold worst-worker p95 of 10.624/63.793 ms, 15,000 durable holds, no overlap, drained queues, and successful rollback.

The 15-minute stage completed 900,000 logical and physical requests with zero generator drops, first-attempt failures, retry attempts, transport errors, or admission rejections. Read/hold worst-worker p95 were 8.808/56.015 ms and all 45,000 acknowledged holds had matching idempotency records and orders. However, the fixed 180-second post-load audit found 2,539 overdue active holds and 751 pending refresh requests, plus one unpublished outbox event. Its expiry/queue gates failed, so the stage did not pass. A later read-only audit found zero overdue holds, zero queue backlog, zero broken links, and zero overlapping seat intervals. The late drain does not retroactively change the failed 180-second gate. No HTTP retry was exercised in either 1,000 RPS stage.
