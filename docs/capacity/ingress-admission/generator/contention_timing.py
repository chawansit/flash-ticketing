...
def app_duration(value, name="app"):
    match = re.search(rf'(?:^|,)\s*{re.escape(name)};dur=([0-9.]+)', value)
...
async def measured_request(client, url, headers, body):
    ...
    try:
        ...
        row = {"status": str(response.status_code), "code": code if response.status_code != 201 else None,
               ...
               "protocol_queue_ms": app_duration(response.headers.get("Server-Timing", ""), "protocol_queue"),
               "asgi_headers_ms": app_duration(response.headers.get("Server-Timing", ""), "asgi_headers"),
    ...
    row["outside_protocol_ms"] = (
        row["client_ms"]-row["protocol_queue_ms"]-row["asgi_headers_ms"]
        if row.get("protocol_queue_ms") is not None and row.get("asgi_headers_ms") is not None else None
    )
    return row
    ...
    try:
        ...
        result = {"contenders": args.contenders, "mode": args.mode, "workers": args.workers,
                  ...
                  "timing_ms": {key: distribution([r.get(key) for r in rows]) for key in
                                ["dispatch_ms","lane_wait_ms","tcp_ms","before_io_ms","send_to_headers_ms","app_ms","protocol_queue_ms","asgi_headers_ms","outside_protocol_ms","outside_app_ms","client_ms","total_ms"]},
        ...
        result["ingress_timing_complete"] = all(r.get("protocol_queue_ms") is not None and r.get("asgi_headers_ms") is not None for r in rows)
        result["measurement_valid"] = result["accounting_pass"] and skew <= 100 and result["warm_reuse_pass"] is not False
        if getattr(args,"require_ingress",False):
            result["measurement_valid"] &= result["ingress_timing_complete"]
...
def main():
    ...
    p.add_argument("--require-ingress", action="store_true")
    a = p.parse_args()
